...
// Function declaration for plotting the path and obstacles
void plotPathAndObstacles(const std::vector<Point>& path, const std::vector<std::pair<Point, Point>>& obstacles_with_velocities,
...
std::vector<Point> apfPathPlanning(const Point& start, const Point& goal, const std::vector<std::pair<Point, Point>>& obstacles_with_velocities,
    ...
    for (int i = 0; i < max_iterations; ++i) {
        ...
        // Plot the path and obstacles after each iteration
        plotPathAndObstacles(path, obstacles_with_velocities, step_size, max_iterations);
...
// Function to plot the path and obstacles
void plotPathAndObstacles(const std::vector<Point>& path, const std::vector<std::pair<Point, Point>>& obstacles_with_velocities ,double step_size, int max_iterations) {
    // Clear previous plot
    plt::clf();
    ...
    for (const auto& obstacle_with_velocity : obstacles_with_velocities) {
        ...
        for (const auto& obstacle_with_velocity : obstacles_with_velocities) {
            ...
            Point current_obstacle_position = obstacle_with_velocity.first;
            for (int i = 0; i < max_iterations; ++i) {
